...
# Seconds to wait between modules to keep the scan low-impact.
REQUEST_DELAY = float(os.environ.get("MORTIS_DELAY", "0.3"))

# Available modules, keyed by the name used in the scan form.
MODULES = {
...
def run_scan(target_url: str, enabled_modules=None, depth: str = "standard", on_progress=None) -> dict:
    ...
    each module starts so the caller can report progress.
    """
